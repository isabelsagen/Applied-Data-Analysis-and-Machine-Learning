...
model = Linear_Regression(x_scaled, y_scaled)
model.generate_data(n, noise=0.1)
model.test_train_split(0.3)  



#----------------------------------------------------------------------------------------------------------
#                        GD and SGD, with and without momentum,  MSE vs learning rate
#----------------------------------------------------------------------------------------------------------
...
s_MSE_wo_mom = np.zeros_like(learning_rate)
s_MSE_w_mom = np.zeros_like(learning_rate)
...
for i in range(len(learning_rate)):
    ...
    s_MSE_wo_mom[i] = model.stochastic_gradient_descent(learning_rate[i], 0, 0, n_epochs, batch_size, 0, 0)[-1]
    s_MSE_w_mom[i] = model.stochastic_gradient_descent(learning_rate[i], 0.9, 0, n_epochs, batch_size, 0, 0)[-1]
    print(i)
...
plt.plot( learning_rate, s_MSE_wo_mom, label=r'SGD, $\gamma  = 0$')
plt.plot(learning_rate, s_MSE_w_mom, label=r'SGD, $\gamma  = 0.9$')
plt.xlabel(r'$\eta$')
plt.ylabel('MSE')
plt.xscale("log")
plt.yscale("log")
...
plt.legend()
plt.tight_layout()
plt.savefig("Lin_GD_SGD_MSE_lerningrate.pdf", format="pdf")
plt.show()
#'''
...
'''   
mse_ols_wo_mom_02 = model.gradient_descent(0.2, n_iterations, 0, lambda_value, 0)
mse_ols_wo_mom_01 = model.gradient_descent(0.1, n_iterations, 0, lambda_value, 0)
mse_ols_wo_mom_001 = model.gradient_descent(0.01, n_iterations, 0, lambda_value, 0)
mse_ols_w_mom_001 = model.gradient_descent(0.01, n_iterations, momentum, lambda_value, 0)

plt.axhline(y=0.01, color='black', linestyle='dotted', label=r'$\sigma^2 =0.01$', zorder=10)
plt.plot(mse_ols_wo_mom_02,label=r'$\eta =0.2$')
plt.plot(mse_ols_wo_mom_01,label=r'$\eta =0.1$')
plt.plot(mse_ols_wo_mom_001,label=r'$\eta =0.01$')
plt.plot(mse_ols_w_mom_001,label=r'$\eta =0.01, \gamma  = 0.9$')
...
plt.legend()
plt.tight_layout()
plt.savefig("Lin_GD_MSE_iterations.pdf", format="pdf")
plt.show()
'''

#----------------------------------------------------------------------------------------------------------
#                  SGD, MSE vs epochs for different optimalisations
#----------------------------------------------------------------------------------------------------------
...
plt.axhline(y=0.01, color='black', linestyle='dotted', label=r'$\sigma^2 =0.01$', zorder=10)
plt.plot(mse_history,label=r'SGD, $\eta =0.01$')
...
plt.tight_layout()
plt.yscale('log')
plt.savefig("Lin_opt.pdf", format="pdf")
plt.show()
'''
...
lambda_value = np.logspace(-10, 0, 11)
...
plt.imshow(MSE_ridge, aspect='auto', cmap='rainbow', norm=colors.LogNorm(),  interpolation='nearest')
...
min_x = min_index[1]
min_y = min_index[0]  
plt.plot(min_x, min_y, 'k*', markersize=15, label=f'Smallest MSE = {min_mse:.3}')

plt.tight_layout()
plt.legend()
plt.savefig("Lin_ridge_lambda_lr.pdf", format="pdf")
plt.show()
'''
...
for i in range(len(n_epochs)):
    for j in range(len(batch_size)):
        MSE[j, i] = model.stochastic_gradient_descent(learning_rate, 0 , 0, n_epochs[i],  batch_size[j], 0, 0)[-1]
        print(i, j)

plt.imshow(MSE, aspect='auto', cmap='rainbow',norm=colors.LogNorm(),  interpolation='nearest')
plt.colorbar(label='MSE')
...
plt.tight_layout()
plt.legend()
plt.savefig("Lin_epoch_minibatch.pdf", format="pdf")
plt.show()
'''
